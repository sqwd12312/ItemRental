...
<body>
	<div>
		...
		<form id="houseForm" name="houseForm"
			action="applyout/findAllApplyoutByZuke.action" method=post>
			<div class="result-title">
								...
								<td>${list.status}</td>
									<td><c:choose>
											<c:when test="${list.status=='申请中'}">
												<a class="link-update"
													href="applyout/agreeApplyOutByUser.action?itemId=${list.itemId }"
													onclick="return window.confirm('确定要同意退租吗？')">同意</a>
											&nbsp;&nbsp; 
											
											<a class="link-del"
													href="applyout/refuseApplyoutByUser.action?itemId=${list.itemId}"
													onclick="return window.confirm('确定要拒绝吗？')">拒绝</a>
											</c:when>
											<c:otherwise>
												<a class="link-del"
													href="applyout/deleteApplyoutByUser.action?aoId=${list.id}"
