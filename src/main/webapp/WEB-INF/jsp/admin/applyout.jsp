...
<div class="admin-content-body">
	...
	<hr>
		...
		<form id="houseForm" name="houseForm"
			action="findallapplyout.action" method=post>
								...
								<td>${list.status}</td>
									<td><c:choose>
											<c:when test="${list.status=='申请中'}">
												<a class="link-update"
													href="agreeApplyOut.action?itemId=${list.itemId }"
													onclick="return window.confirm('确定要同意退租吗？')">同意</a>
											&nbsp;&nbsp; 
											
											<a class="link-del"
													href="refuseapplyout.action?itemId=${list.itemId}"
													onclick="return window.confirm('确定要拒绝吗？')">拒绝</a>
											</c:when>
											<c:otherwise>
												<a class="link-del"
													href="adminDeleteApplyout.action?aoid=${list.id}"
