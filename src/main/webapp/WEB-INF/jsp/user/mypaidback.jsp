...
<div class="admin-content-body">
	<div class="am-cf am-padding am-padding-bottom-0">
		<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">租金退回列表</strong><small></small></div>
    </div>
	<hr>
</div>
<div class="search-wrap">
            <div class="search-content">
                <form action="getMyPaidBack.action" method="post" name="myform">
					...
					<div class="result-content">
						<table id=grid
							...
							<tbody>
								...
								<c:forEach items="${paids}" var="paidList">
									<tr
										style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
										<td>${paidList.paid.id}</td>
										<td>${paidList.paid.itemNumber}</td>
										<td>${paidList.paid.itemName}</td>										
										<td>${paidList.paid.price}</td>
										<td>${paidList.paid.paydate}</td>
										<td>${paidList.zuke.name}</td>										
										<td>${paidList.chuzu.name}</td>
										<td>${paidList.chuzu.phone}</td>
										<td>${paidList.paid.status}</td>
										<td>
										
										<a class="link-update"
											href="deletePaidBack.action?id=${paidList.paid.id}"
