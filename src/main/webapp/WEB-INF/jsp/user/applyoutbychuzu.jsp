...
<div class="admin-content-body">
	<div class="am-cf am-padding am-padding-bottom-0">
		<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">退租申请</strong><small></small></div>
    </div>
	<hr>
</div>
		<form id="houseForm" name="houseForm"
			action="findAllApplyoutByZuke.action" method=post>
								...
								<td>${list.status}</td>
									<td><c:choose>
											<c:when test="${list.status=='申请中'}">
												<a class="link-update"
													href="agreeApplyOutByUser.action?itemId=${list.itemId }"
													onclick="return window.confirm('确定要同意退租吗？')">同意</a>
											&nbsp;&nbsp; 
											
											<a class="link-del"
													href="refuseApplyoutByUser.action?itemId=${list.itemId}"
													onclick="return window.confirm('确定要拒绝吗？')">拒绝</a>
											</c:when>
											<c:otherwise>
												<a class="link-del"
													href="deleteApplyoutByUser.action?aoId=${list.id}"
